{{
  tag="event"
  display="list"
  order="recent"
  howmany="2"
  before_show="<div class='widget events div-col-3'>"
  show="<article class='div-col'>"
  show="<a href='__url__'>"
  show="<div class='date-wrap'>"
  show="<span class='month'>__eventstart format='M'__</span>"
  show="<span class='day'>__eventstartTwo format='j'__</span>"
  show="</div>"
  show="<h3>__title__</h3>"
  show="<p class='meta'>"
  show="<span class='times'>__eventtimes__</span>"
  show="<span class='location'>__location__</span>"
  show="</p>"
  show="</a>"
  show="</article>"
  after_show="<script type="text/javascript"> //<![CDATA[ $(document).ready(function(){ var pageGroup = window.location.pathname.split('/')[2]; var eventCategoryLink = $('.event-category-link'); eventCategoryLink.attr('href', eventCategoryLink.attr('href') + pageGroup); }); // ]]> </script>" 
  after_show="<article class='div-col'>"
  after_show="<a href='/happenings/events/list/category/'>"
  after_show="<div class='date-wrap'>"
  after_show="<span class='month'>Get</span>"
  after_show="<span class='day'>All</span>"
  after_show="</div>"
  after_show="<h3>Events</h3>"
  after_show="<p class='meta'>"
  after_show="<span class='times'>Like These</span>"
  after_show="</p>"
  after_show="</a>"
  after_show="</article>"
  after_show="</div>"
}}